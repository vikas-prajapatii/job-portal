...
export default function ApplyJob() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (portfolioUrl.trim() || screeningAnswer.trim()) {
        finalCoverLetter += "\n\n--- Additional Details ---";
        if (portfolioUrl.trim()) finalCoverLetter += `\nPortfolio/Video Resume URL: ${portfolioUrl.trim()}`;
        if (screeningAnswer.trim()) finalCoverLetter += `\nWhy should we hire you?: ${screeningAnswer.trim()}`;
      }

      await dispatch(submitApplication({
        jobId: Number(id),
        resumeId: Number(selectedResume) || null,
        coverLetter: finalCoverLetter || null,
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 3: return <AdditionalDetails expectedSalary={expectedSalary} setExpectedSalary={setExpectedSalary} availableFrom={availableFrom} setAvailableFrom={setAvailableFrom} portfolioUrl={portfolioUrl} setPortfolioUrl={setPortfolioUrl} screeningAnswer={screeningAnswer} setScreeningAnswer={setScreeningAnswer} />
      case 4: return <ReviewSubmit resume={selectedResume} resumes={resumes} coverLetter={coverLetter} expectedSalary={expectedSalary} availableFrom={availableFrom} job={job} portfolioUrl={portfolioUrl} screeningAnswer={screeningAnswer} />
